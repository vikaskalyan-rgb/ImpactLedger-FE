...
const createHyphenator = (source) => {
            ...
            for (let end = start; end < length; end += 1) {
                ...
                if (plain) {
                    for (let p = 0; p < plain.length; p += 1) {
                        ...
                        for (let l = 0; l < entryLevels.length; l += 1) {
                            ...
                            if (level > levels[start + l])
                                levels[start + l] = level;
                        }
                    }
                }
                ...
                if (anchored) {
                    for (let p = 0; p < anchored.length; p += 1) {
                        ...
                        if (entry.anchorStart && start !== 0)
                            continue;
                        if (entry.anchorEnd && start + entry.length !== length)
                            continue;
    ...
    const hyphenate = (word) => syllables(word).join(SOFT_HYPHEN);
    return { hyphenate, syllables };
};

export { createHyphenator as default };
