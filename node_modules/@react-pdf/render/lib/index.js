...
const expandSmoothQuadratics = (d) => {
    const segments = absPath(parsePath(d));
    let x = 0;
    let y = 0;
    let startX = 0;
    let startY = 0;
    ...
    return out.map((seg) => seg[0] + seg.slice(1).join(' ')).join('');
};
...
const renderGlyphs = (ctx, glyphs, positions, x, y) => {
    if (ctx.glyphs)
        return ctx.glyphs(glyphs, positions, x, y);
...
const bisectAngle = (inAngle, outAngle) => {
...
const getLineMarkerPoints = (node) => {
    const x1 = node.props?.x1 || 0;
    const y1 = node.props?.y1 || 0;
    const x2 = node.props?.x2 || 0;
    const y2 = node.props?.y2 || 0;
...
const getPointListMarkerPoints = (points, closed) => {
    if (points.length === 0)
        return {};
    if (points.length === 1)
...
const getPolylineMarkerPoints = (node) => {
    const points = parsePoints(node.props?.points);
    return getPointListMarkerPoints(points, false);
};
const getPolygonMarkerPoints = (node) => {
    const points = parsePoints(node.props?.points);
    return getPointListMarkerPoints(points, true);
};
const getPathMarkerPoints = (node) => {
    const d = node.props?.d;
    if (!d)
        return {};
...
const markerPointFns = {
    [P.Line]: getLineMarkerPoints,
    [P.Polyline]: getPolylineMarkerPoints,
    [P.Polygon]: getPolygonMarkerPoints,
    [P.Path]: getPathMarkerPoints,
};
const getMarkerPoints = (node) => {
...
const drawMarkerChildren = (ctx, node, drawNode) => {
    const children = node.children || [];
...
const renderMarkerInstance = (ctx, marker, point, strokeWidth, isStart, drawNode) => {
...
const hasMarkers = (node) => {
    if (!node.props)
        return false;
    return (('markerStart' in node.props && !!node.props.markerStart) ||
...
const renderMarkers = (ctx, node, drawNode) => {
    if (!hasMarkers(node))
        return;
    if (!MARKER_TYPES.has(node.type))
        return;
    const points = getMarkerPoints(node);
    if (!points)
        return;
    const strokeWidth = ('strokeWidth' in (node.props || {}) && node.props.strokeWidth) ||
        1;
    const props = node.props || {};
    if ('markerStart' in props && props.markerStart && points.start) {
        renderMarkerInstance(ctx, props.markerStart, points.start, strokeWidth, true, drawNode);
    }
    ...
    if ('markerEnd' in props && props.markerEnd && points.end) {
        renderMarkerInstance(ctx, props.markerEnd, points.end, strokeWidth, false, drawNode);
    }
};
...
const drawImage = (ctx, options) => {
    const { x, y, width: containerWidth, height: containerHeight, image, opacity, objectFit, objectPositionX, objectPositionY, imageCache = new Map(), } = options;
...
const renderImageBackground = (ctx, node, options) => {
    if (!node.box)
        return;
    if (!node.image)
        return;
...
const debugHitSlop = (ctx, node) => {
    if (!node.box || !node.props)
        return;
    if (!('hitSlop' in node.props))
        return;
    const slop = resolveHitSlop(node.props.hitSlop);
    if (!slop.top && !slop.bottom && !slop.left && !slop.right)
        return;
    const { left, top, width, height } = node.box;
    ...
    if (slop.top) {
        ctx
            .rect(left - slop.left, top - slop.top, width + slop.left + slop.right, slop.top)
            .fill();
    }
    // Bottom
    if (slop.bottom) {
        ctx
            .rect(left - slop.left, top + height, width + slop.left + slop.right, slop.bottom)
            .fill();
    }
    // Left
    if (slop.left) {
...
const fillBorderTop = (ctx, layout, style, rtr, rtl) => {
    ...
    const { borderTopColor, borderTopColorOpacity, borderTopWidth, borderTopStyle, borderRightWidth, borderLeftWidth, } = style;
...
const fillBorderRight = (ctx, layout, style, rtr, rbr) => {
    const { top, left, width, height } = layout;
    const { borderRightColor, borderRightColorOpacity, borderRightStyle, borderRightWidth, borderTopWidth, borderBottomWidth, } = style;
    const c0 = rbr * (1.0 - KAPPA);
    const c1 = rtr * (1.0 - KAPPA);
...
const fillBorderBottom = (ctx, layout, style, rbl, rbr) => {
    const { top, left, width, height } = layout;
    const { borderBottomColor, borderBottomColorOpacity, borderBottomStyle, borderBottomWidth, borderRightWidth, borderLeftWidth, } = style;
    const c0 = rbl * (1.0 - KAPPA);
...
const fillBorderLeft = (ctx, layout, style, rbl, rtl) => {
    ...
    const { borderLeftColor, borderLeftColorOpacity, borderLeftStyle, borderLeftWidth, borderTopWidth, borderBottomWidth, } = style;
    const c0 = rbl * (1.0 - KAPPA);
...
const renderBorders = (ctx, node) => {
    ...
    const { borderTopColor = 'black', borderTopStyle = 'solid', borderLeftColor = 'black', borderLeftStyle = 'solid', borderRightColor = 'black', borderRightStyle = 'solid', borderBottomColor = 'black', borderBottomStyle = 'solid', } = node.style;
    ...
    const borderTopColorOpacity = Math.min(parsedBorderTopColor.opacity, nodeOpacity);
    const borderRightColorOpacity = Math.min(parsedBorderRightColor.opacity, nodeOpacity);
    const borderBottomColorOpacity = Math.min(parsedBorderBottomColor.opacity, nodeOpacity);
    const borderLeftColorOpacity = Math.min(parsedBorderLeftColor.opacity, nodeOpacity);
    // @ts-expect-error this is always a number due to resolve border radius step
 ...
 */
const addFormAnnotation = (ctx, name, type, box, options, extra) => {
...
const renderTextInput = (ctx, node, options) => {
    ...
    const { MaxLen, ...fieldOptions } = parseTextInputOptions(node, fieldSetOptions);
...
const renderCheckbox = (ctx, node, options) => {
    ...
    const { AP, AS, ...fieldOptions } = parseCheckboxOptions(ctx, node, fieldSetOptions);
